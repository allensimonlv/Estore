...
public class TransactionManager {
	...
	public static void startTran() throws SQLException{
		...
		realconn_local.set(conn);//--为了方便后续关闭连接,将这个连接保存起来，保存在当前线程中
		//--由于一个事务需要执行多条sql,每个sql执行过后都关闭连接,这样一来后续的sql没法执行,所以这个地方要改造close方法,使他不能关闭连接
	...
	public static void release(){
		...
		//--之前连接是没有关闭的，因为在开启事务的方法中，把close默认关闭的功能给取消了。在release的时候才是真正的关闭连接
